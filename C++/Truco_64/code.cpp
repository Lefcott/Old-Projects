...
bool env_disp(int nEnv/*Número de envido*/), envDisp2()/*Dice si se puede cantar algún envido*/;
...
void mat_min(), COO2(), pedir_tecla(int cantar), borrar(), resp_env(), env_res_usu(), comment_clean(), env_proc(char tec, bool mano_env);
void pensar(), jugada_maquina(), DTT(), jugar_usuario(), jugar_maquina(), setcursor(bool visible, DWORD size);
int cant(int rango), MMM(int ret), COO(), car_disp(), env_fl(int JM), pun_gan(), suma_tantos();
	...
	if(AL <= prob / 2 && (env_disp(1) || env_disp(2) || env_disp(3))){//Cantar envido.
		...
		if(en_resp){
			...
			if(AL <= prob / 2 && env_fl(2) >= 20){
				...
				aux(66, 37); printf("Enter: %s", (!UGE) ? (env_prd) ? "me ganaste de mano." : "son buenas." : "");
				aux(66, 37); if(UGE) printf("Enter: %d %s.", env_fl(1), (env_prd) ? "y soy mano." : "son mejores.");
				fin_env = 0;
				pU += (UGE) ? tantos : 0;
				pM += (!UGE) ? tantos : 0;
				system("color B2");
...
void env_proc(char tec, bool mano_env){
	...
	if(tec == key_F){
		if(!mano_env){
			...
			if(!flor){
				...
				comment_clean();
				if(env_fl(2) > 26){
					aux(30, 37); printf("QUIERO, %d!", m_fl);
	...
	if(tec == 13){
		if(!flor){
			...
			Sleep(650);
			aux(66, 37); printf("                   ");
			aux(66, 37); printf("%s...", (rndRs == 0) ? "Eh" : (rndRs == 1) ? "MMM" : "");
			aux(66, 37); printf("%s", (UGE) ? (env_prd) ? "Me ganaste de mano." : "Son buenas." : "");
...
void tomar_tecla(){
	...
	car = toupper(getch());
	env_fl(1);
	if((car == key_E && env_disp(1)) || (car == key_R && env_disp(2)) || (car == key_F && env_disp(3))) env_proc(car, flor);
	if(tab == 1){
		if(car == 75){
			if(selec == 2){
